...
protected:
    ...
    float suprafata_utila;//suprafata utila
...
class Casa:public Locuinta
{
    ...
    float pretm2curte;//pret pe metru patrat curte
    int nr_etaje;//numarul de etaje
    float *et;//retine suprafata utila pentru fiecare etaj in parte
public:
    istream& citire(istream&);//functie de citire supraincarcata
    ostream& afisare(ostream&) const;//functie de afisare supraincarcata
    void calc_chirie();//calculeaza chiria
    ...
    Casa():Locuinta(){this->curte=0;this->pretm2curte=0;this->nr_etaje=0;this->et=NULL;calc_chirie();}//constructor fara parametri
    ...
    {this->curte=c;this->pretm2curte=pm2curte;this->nr_etaje=nr_et;cout<<"Introduceti suprafata utila pt fiecare etaj in parte:"<<endl;this->et=new float[this->nr_etaje];
     for(int i=0;i<this->nr_etaje;i++)cin>>this->et[i]; this->calc_chirie();}//constructor parametrizat
...
///Specializarea clasei Gestiune pentru char*

template<>
class Gestiune<char*>
{
    vector<Locuinta*> p;//vector ce retine locuintele aflate in gestiune
    vector<string> nume_clienti;//vector ce retine numele clinetilor care au inchiriat casele aflate in gestiune
public:
    Gestiune();//constructor de initializare
    Gestiune(int nrl);//constructor de initializare parametrizat
    Gestiune(const Gestiune<char*>&);//constructor de copiere
    Gestiune<char*> operator=(const Gestiune<char*>&);//operator de atribuire

    Locuinta* operator+=(Locuinta*);//operator += supraincarcat care adauga o noua locuinta in gestiune

    friend istream& operator>>(istream&,Gestiune<char*>&);//operator de citire supraincarcat
    friend ostream& operator<<(ostream& ,const Gestiune<char*>&);//operator de afisare supraincarcat
};
...
istream& Apartament::citire(istream& in)//functie de citire supraincarcata
{
    ...
    etajexc:
        try//se citeste etajul la care se afla apartamentul si se verifica daca este un nr pozitiv <2000
        {
            cout<<"etaj:";
            in>>this->etaj;
...
istream& Casa::citire(istream& in)//functie de citire supraincarcata
{
    ...
    curteexc:
        ...
        }catch(float c)
        {
            ...
            goto curteexc;//se cere reintroducerea suprafetei curtii
        }
    ...
    pretm2curteexc:
        ...
        }catch(float c)
        {
            ...
            goto pretm2curteexc;//se cere reintroducerea pretului pe metru patrat de curte
        }
    ...
    numaretajeexc:
        ...
        }catch(int c)
        {
            ...
            goto numaretajeexc;//se cere reintroducerea numarului de etaje
        }
    ...
    suppeetexc:
        ...
        }catch(float sss)
        {
            ...
            goto suppeetexc;//se cere reintroducerea suprafetei pe etaj
        }catch(int x)
        {
...
{
    out<<"Casa: ";
    out<<"Nume client- "<<this->nume_client<<"; Suprafata utila- "<<this->suprafata_utila<<"; Discount- "<<this->discount<<";Pret pe metru patrat- "<<this->pretm2<<"; Suprafata curte- "<<this->curte<<"; Pret pe metru patrat curte- ";
...
template<class T>
Gestiune<T>::Gestiune(int nrl)//constructor de initializare paramtrizat
{
...
istream& operator>>(istream& in, Gestiune<T> & ob)//operator de citire supraincarcat
{
    ...
    int nrl;
    in>>nrl;//citeste numarul de locuinte care trb adaugate in gestiune
...
///Metodele clasei Gestiune<char*>

Gestiune<char*>::Gestiune()//constructor de initializare
{
    p.resize(0);
    nume_clienti.resize(0);
}

Gestiune<char*>::Gestiune(int nrl)//constructor de initializare parametrizat
{
...
Gestiune<char*>::Gestiune(const Gestiune<char*>& ob)//constructor de copiere
{
    p.assign(ob.p.begin(),ob.p.end());
...
Gestiune<char*> Gestiune<char*>::operator=(const Gestiune<char*>& ob)//operator de atribuire
{
    p.assign(ob.p.begin(),ob.p.end());
    nume_clienti.assign(ob.nume_clienti.begin(),ob.nume_clienti.end());
    return ob;
 ...
 Locuinta* Gestiune<char*>::operator+=(Locuinta* ob)//operator += supraincarcat
 {
    Apartament a;
    Casa b;
...
istream& operator>>(istream& in,Gestiune<char*>& ob)//operator de citire supraincarcat
{
    cout<<"Introduceti numarul de locuinte care sa se afle in gestiune ( Gestiune<char*>)";
    int nrl;
    in>>nrl;//citeste numarul de locuinte din gestiune
    ...
    for(int i=0;i<nrl;i++)//citeste locuintele
    {
        cout<<"Introduceti datele despre locuinta "<<i+1<<" ( A - apartament , C - casa)"<<endl;
        in>>opt;
        switch(opt)
        {
        case 'A':
...
ostream& operator<<(ostream& out,const Gestiune<char*>& ob)//operator de afisare;
{
    out<<"Se afiseaza locuintele aflate in gestiune:"<<endl;
    Apartament a;
    Casa b;
    ...
    out<<"Numele clientilor care au inchiriat locuintele aflate in gestiune:"<<endl;
